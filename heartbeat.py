...
TOOLS = [
    ...
    {
        "name": "push_intent",
        "description": "Start a sustained behaviour. Two independent stacks run in parallel — nav (wheels/body) and attention (gimbal) — and each intent is automatically routed to the right one based on which actuators it uses. Nav intents: drive_distance (closed-loop on odometry), turn_to_heading (closed-loop on magnetometer), face_gimbal (turn body to match gimbal, then centre gimbal), follow (walk alongside a tracked person using OAK-D spatial detections — auto-aligns body first if gimbal is off-centre), approach (drive up to a detected subject and STOP at a standoff distance — the closed-loop 'go to that thing' primitive; same spatial detections + body auto-align as follow), go_forward (open-loop), turn (open-loop), sit (stay and observe), explore (wander). Attention intents: scan (sweep gimbal across FOV), look_at (point gimbal). Because the stacks are independent, you can run one nav + one attention in parallel — e.g. `follow` a person while `scan` sweeps for other things. The clean pattern to go toward a place: 1) look or scan to find it with the gimbal, 2) face_gimbal to align the body, 3) drive_distance to go there. To walk *with* someone rather than to a place, use follow. To drive up to a specific detected subject and stop on it, use approach — it's closed-loop on the live detection, unlike drive_distance which drives a blind fixed distance.",
        "input_schema": {
            "type": "object",
            "properties": {
                "intent": {
                ...
                "params": {
                    "type": "object",
                    "description": "Parameters. drive_distance: {distance (m, negative=reverse), speed, timeout}. turn_to_heading: {target_heading (compass deg) OR relative_turn (deg, +ve=right), speed, timeout}. face_gimbal: {timeout}. scan: {range (deg, default 60), steps (default 5), tilt (default -10), hold (sec per position, default 2)}. follow: {target_id (string — the track_id from the spatial_detections list, this is the SEMANTIC way to pick), distance (metres to maintain, default 1.0), max_speed (default 0.15). target_index is legacy/fallback — avoid it; it picks by position in the list and gets confused when the first detection is Chopper or a YOLO-mislabelled object rather than the person you meant}. approach: {target_id (track_id from spatial_detections — semantic, preferred), standoff (metres to stop short of the target, default 1.0), max_speed, timeout}. go_forward: {speed, duration}. turn: {direction, speed, duration}. look_at: {pan, tilt}. sit: {duration, reason}. explore: {speed, duration, interval}."
                }
            },
            "required": ["intent"]
        }
